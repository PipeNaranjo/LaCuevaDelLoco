...
public class RegistroDeVentas implements Serializable {
	...
	// es el valor que se le asignara en el siguiente informe fiscal que se
	// genere
	private int codigoInformeFiscal;
	// es la lista de recibos guardad en las pociciones que corresponde a a�o,
	// mes y dia del recibo
	private List<List<List<List<Recibo>>>> recibos;
	// lista de informes fiscales generados con anterioridad
	private List<InformeFiscal> informesFiscales;
	// lista de recibos que estan pendientes por incluir en el siguiente informe
	// fiscal
	 ...
	 * 
	 * @param codigoRecibo
	 *            el codigo de barras con el cual empezaran a contar los recibos
	 * @param codigoInformeFiscal
	 *            codigo con el cual empezaran a generarse los informes fiscales
	 */
	public RegistroDeVentas(int codigoRecibo, int codigoInformeFiscal) {
		...
		recibos = new ArrayList<List<List<List<Recibo>>>>();
	 ...
	 * 
	 * @param recibo
	 *            recibo el cual se quiere adjuntar
	 * @param inventario
	 *            inventario de los productos en donde restar los productos
	 *            vendidos
	 * @return regresa un boolena que indica que se ha a�adido el recibo con
	 *         exito
	 */
	 ...
	 * 
	 * @param fecha
	 *            la fecha de la que se quiere obtenr los recibos en formato
	 *            GregorianCalendar
	 * @return regresa la lista de fechas
	 */
	public List<Recibo> obtenerListaDeRecibos(GregorianCalendar fecha) {
	 ...
	 * 
	 * @param fecha
	 *            la fecha en la cual se busca el recibo
	 * @param codigoDeBarras
	 *            codigo de barras del recibo a buscar
	 * @return Recibo que contiene el codigo de barras especificado null si no
	 *         se encuentra el recibo en esa fecha
	 */
	public Recibo obtenerRecibo(GregorianCalendar fecha, String id) {
		List<Recibo> lista = obtenerListaDeRecibos(fecha);
		for (Recibo i : lista) {
	...
	public boolean removerRecibo(GregorianCalendar fecha, String id) {
		...
		if (r != null) {
			obtenerListaDeRecibos(fecha).remove(r);
			// recibos.remove(r);
			return true;
		}
		return false;
	...
	/**
	 * metodo que cierra el informe fiscal con los productos que aun estan
	 * pendientes para ser agregados a un informe
	 * 
	...
	/**
	 * metodo que sirve para verificar si el a�o ha sido creado, y esta lista
	 * para el uso de lo contrario creara los a�os necesarios para llegar hasta
	 * este.
	 * 
	 * @param fecha
	 *            la fecha que se quiere verificar
	 */
	private void verificarAnio(GregorianCalendar fecha) {

		while (recibos.size() < fecha.get(Calendar.YEAR) - 2019) {
			recibos.add(new ArrayList<List<List<Recibo>>>());
			for (int i = 0; i < 12; i++) {
				recibos.get(recibos.size() - 1).add(new ArrayList<List<Recibo>>());
	 ...
	 * 
	 * @param codigoRecibo
	 *            nuevo codigo de barras para poner en el siguiente recibo
	 */
	 ...
	 * 
	 * @param codigoRecibo
	 *            nuevo codigo de barras para poner en el siguiente fiscal
	 */
	 ...
	 */
	public List<List<List<List<Recibo>>>> getRecibos() {
		return recibos;
	}
	 ...
	 * 
	 * @param recibos
	 *            la nueva lista de recibos
	 */
	public void setRecibos(List<List<List<List<Recibo>>>> recibos) {
	 ...
	 */
	public List<InformeFiscal> getInformesFiscales() {
	 ...
	 * 
	 * @param informesFiscales
	 *            nueva lista de Informes Fiscales
	 */
	public void setInformesFiscales(List<InformeFiscal> informesFiscales) {
	 ...
	 */
	public List<Recibo> getRecibosPendientes() {
	 ...
	 * 
	 * @param recibosPendientes
	 *            nueva lista de recibos pendientes por informe fiscal
	 */
	public void setRecibosPendientes(List<Recibo> recibosPendientes) {
